...
class ParenthesisRule:
    ...
    def classify(self, tweet: str, positive_pattern: str, negative_pattern: str) -> int:
    ...
    def heart_rule(self, tweet: str) -> int:
        return self.classify(tweet, "< 3", "< / 3")

    def happy_sad_hashtag_rule(self, tweet: str) -> int:
        return self.classify(tweet, "#happy", "#sad")
...
class Rule(ParenthesisRule):
    ...
    def predict(self, tweets: List[str]) -> np.ndarray:
        ...
        predictions = [
            self.aggregate(per_tweet_predictions)
    ...
    def aggregate(self, predictions: List[int]) -> int:
